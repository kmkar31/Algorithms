...
public class KdTree {
    ...
    private Node put(final Node node, final Point2D p, final int level,
                     final RectHV rect) {
        if (node == null) {
        ...
        if (cmp < 0 && node.left == null) {
            if (level % 2 == 0) {
                rectLeft = new RectHV(node.rect.xmin(), node.rect.ymin(), node.point.x(),
                                      node.rect.ymax());
            }
            else {
                rectLeft = new RectHV(node.rect.xmin(), node.rect.ymin(), node.rect.xmax(),
                                      node.point.y());
            }
        }
        else if (cmp >= 0 && node.right == null) {
            if (level % 2 == 0) {
                rectRight = new RectHV(node.point.x(), node.rect.ymin(), node.rect.xmax(),
                                       node.rect.ymax());
            }
            else {
                rectRight = new RectHV(node.rect.xmin(), node.point.y(), node.rect.xmax(),
                                       node.rect.ymax());
            }
        }

        if (cmp < 0) node.left = put(node.left, p, level + 1, rectLeft);
        else if (cmp > 0) node.right = put(node.right, p, level + 1, rectRight);
    ...
    private Point2D get(final Node node, final Point2D searchedPoint, final int level) {
        ...
        if (cmp < 0) return get(node.left, searchedPoint, level + 1);
            ...
            return get(node.right, searchedPoint, level + 1);
    ...
    public Iterable<Point2D> range(final RectHV queryRect) {
        if (queryRect == null) throw new IllegalArgumentException("Empty Search Rectangle");
    ...
    public Point2D nearest(final Point2D queryPoint) {
        if (queryPoint == null) throw new IllegalArgumentException("Empty Query Point");
        if (root == null) {
            return null;
        }
    ...
    private Point2D nearest(final Point2D queryPoint, final Node node,
                            final Point2D currentlyClosestPoint,
